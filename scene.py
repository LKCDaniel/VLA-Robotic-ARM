...
class SimScene:
    ...
    def update_frame(self, dx, dy, dz): # -> bool:
        ...
        if self.stick_state == self.StickState.Stationary:
            ...
            if (
                self.stick.rotation_euler[2]%(2*math.pi) > local_angle%(2*math.pi) > (self.stick.rotation_euler[2]-self.thickness_angle)%(2*math.pi)
                and self.robot_arm.location[2] < 0.8682
            ): # 0.15/sin(10 degrees) height
